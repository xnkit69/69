...
function BootingScreen(props) {
    ...
    return (
        <div style={(props.visible || props.isShutDown ? { zIndex: "100" } : { zIndex: "-20" })} className={(props.visible || props.isShutDown ? " visible opacity-100" : " invisible opacity-0 ") + " absolute duration-500 select-none flex flex-col justify-around items-center top-0 right-0 overflow-hidden m-0 p-0 h-screen w-screen bg-black"}>
            <img width="400px" height="400px" className="md:w-1/4 w-1/2" src="./themes/Yaru/status/cof_orange_hex.svg" alt="X[69] Logo" />
            <div className="w-10 h-10 flex justify-center items-center rounded-full outline-none cursor-pointer" onClick={props.turnOn} >
                {(props.isShutDown
                    ...
                    : <img width="40px" height="40px" className={" w-10 " + (props.visible ? " animate-spin " : "")} src="./themes/Yaru/status/process-working-symbolic.svg" alt="X[69] Process Symbol" />)}
            </div>
            <img width="200px" height="100px" className="md:w-1/5 w-1/2" src="./themes/Yaru/status/ubuntu_white_hex.svg" alt="X[69] Name" />
            <div className="text-white mb-4">
                <a className="underline" href="https://instagram.com/xnkit_k/" rel="noreferrer noopener" target="_blank">Instagram</a>
                <span className="font-bold mx-1">|</span>
                <a href="https://github.com/xnkit69" rel="noreferrer noopener" target="_blank" className="underline">github</a>
            </div>
        </div>
    )
}
